...
import { View, Text, StyleSheet, ScrollView, Pressable, TextInput, Alert, Modal, ActivityIndicator, Switch, KeyboardAvoidingView, Platform } from 'react-native';
...
interface Course {
  ...
  max_participants: number; trainer_id: string; category: string | null; is_active: boolean;
}
...
export default function AdminCoursesScreen() {
  ...
  const openNew = () => {
    setEditing(null);
    setForm({ name: '', description: '', duration_minutes: '60', max_participants: '10', category: '', is_active: true });
    setModalOpen(true);
  };
  ...
  const softDelete = (c: Course) => {
    confirmAlert('Kurs deaktivieren?', c.name, async () => {
      ...
      <ScrollView contentContainerStyle={{ padding: Spacing.md, paddingTop: Spacing.lg, paddingBottom: Spacing.xxl + Spacing.lg }}>
        ...
        {courses.map(c => (
          <View key={c.id} style={styles.card}>
            ...
            <Pressable onPress={() => softDelete(c)}><Text style={[styles.link, { color: Colors.error }]}>X</Text></Pressable>
